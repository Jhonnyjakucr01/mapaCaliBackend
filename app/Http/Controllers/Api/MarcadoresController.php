...
class MarcadoresController extends Controller
{
    ...
    public function procesarClusteringCali(Request $request)
    {
        try {
            if (!$request->hasFile('archivo')) {
                return response()->json(['error' => 'No se encontró ningún archivo'], 400);
            ...
            $archivo = $request->file('archivo');
            $rutaArchivo = storage_path('app/clustering_cali.xlsx');
            $archivo->move(storage_path('app'), 'clustering_cali.xlsx');

            $rutaScript = base_path('app/Http/Controllers/Api/cluster.py');
            $pythonPath = 'C:\Users\User\AppData\Local\Programs\Python\Python313\python.exe'; // Ajusta según sea necesario
            ...
            if (!$process->isSuccessful()) {
                info("❌ Error en el script Python: " . $process->getErrorOutput());
                throw new ProcessFailedException($process);
            }

            info("✅ Respuesta desde Python: " . $process->getOutput());
            ...
            if (!$json) {
                return response()->json([
                    'error' => 'No se pudo decodificar la salida del script Python',
                    'raw_output' => $output,
                ], 500);
            }
